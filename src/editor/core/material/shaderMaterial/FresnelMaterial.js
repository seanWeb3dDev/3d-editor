...
export class FresnelMaterial extends THREE.ShaderMaterial {
    constructor(setting = {}) {
        ...
        this.uniforms.strength = uniforms.strength !== undefined ? { value: uniforms.strength } : { value: 0.85 };

        this.vertexShader = this.createVerTexShader();
        this.fragmentShader = this.createFragmentShader();

        this.shaderType = "FresnelMaterial";
...
//   float intensity = 1.0 - dot( mNormal,viewDir);
  float intensity = 1.0 - rez;
  gl_FragColor = vec4(color,pow(intensity,strength));
        }`;
    }
}
